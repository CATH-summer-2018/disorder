...
def write_file(savedname, data):
    with open(savedname, 'w') as f:
        f.write(data)

def run_iupred_rework(input_file, savedname, type='long'):
    sequence = read_seq(input_file)
    iupred2_result = iupred(sequence, type)
    if '-a' in sys.argv:
        if type == 'long':
    ...
    if type == 'glob':
        write_file(savedname, iupred2_result[1])
    for pos, residue in enumerate(sequence):
        write_file(savedname, '{}\t{}\t{:.4f}'.format(pos + 1, residue, iupred2_result[0][pos]))
        if '-a' in sys.argv:
            write_file(savedname, "\t{:.4f}".format(anchor2_res[pos]))
        write_file(savedname, "\n")
